...
/*   Updated: 2023/07/31 13:25:14 by wricky-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // if the form has not been signed
    if (getIsSigned() == false)
    {
        throw Form::FormNotSignedCannotExecuteException();
        return ;
    }

    // check if the executor's grade is too low
...
void    Form::executeAction(const Bureaucrat &executor) const
{
    (void)executor;
    std::cout << "There's nothing I can do with this form." << std::endl;
}
...
const char *Form::FormNotSignedCannotExecuteException::what() const throw()
{
    return RED "[FORM EXCEPTION] FORM NOT SIGNED! CANNOT PROCEED!" RESET;
}
