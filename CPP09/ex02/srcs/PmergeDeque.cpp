...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/07 12:09:00 by wricky-t          #+#    #+#             */
/*   Updated: 2023/09/07 14:04:20 by wricky-t         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "PmergeMe.hpp"

typedef std::list<GroupIterator<IntDeqIte> > MainChain;
 ...
 */
static void sortAndSwapPairs(GroupIterator<IntDeqIte> begin, GroupIterator<IntDeqIte> end)
{
    for (GroupIterator<IntDeqIte> it = begin; it != end; it += 2)
    {
        if (*(it + 1) < *it)
            swapRange(it, it + 1);
    }
 ...
 */
static void setupChainAndPend(GroupIterator<IntDeqIte> begin, size_t size, MainChain &mainChain, Pend &pend)
{
 ...
 */
static bool comparator(const GroupIterator<IntDeqIte> &lhs, const int &rhs)
{
    return *lhs < rhs;
}
 ...
 */
static void insertPendToMainChain(MainChain &mainChain, Pend &pend, GroupIterator<IntDeqIte> &begin, bool hasLeftover, GroupIterator<IntDeqIte> &leftOver)
{
    Pend::iterator currentPend = pend.begin();
    GroupIterator<IntDeqIte> pendValue = begin + 2; // the 2nd smaller value in all pairs
 ...
 */
static void syncChanges(MainChain &mainChain, GroupIterator<IntDeqIte> &begin)
{
    IntVect cache;
    ...
    for (MainChainIte it = mainChain.begin(); it != mainChain.end(); ++it)
    {
        ...
        GroupIterator<IntDeqIte> groupIter = *it;

        // Extract the begin and end iterators for the group
        IntDeqIte begin = groupIter.base();
        IntDeqIte end = begin + groupIter.size();
 ...
 */
static void fordJohnsonSort(GroupIterator<IntDeqIte> begin, GroupIterator<IntDeqIte> end)
{
    ...
    GroupIterator<IntDeqIte> endIte = hasLeftover ? --end : end;

    // pair elements into n/2 pairs and sort them pairwise
    sortAndSwapPairs(begin, endIte);

    // recursively create pairs and sort
    fordJohnsonSort(GroupIterator<IntDeqIte>(begin, begin.size() * 2), GroupIterator<IntDeqIte>(end, end.size() * 2));
...
/**
 * FordJohnson Algo Runner for Deque
 */
void dequeRunner(const IntVect &original, double parsingTime)
{
    ...
    std::cout << BGREEN << "Before (DEQUE):" << RESET << std::endl;
    showElements<IntDeq>(elements);

    fordJohnsonSort(GroupIterator<IntDeqIte>(elements.begin(), 1), GroupIterator<IntDeqIte>(elements.end(), 1));

    endTime = getTimeStamp();
    elapsedTime = (endTime - startTime) + parsingTime;

    std::cout << BGREEN << "After (DEQUE):" << RESET << std::endl;
    showElements<IntDeq>(elements);
    showTimeStamp(elements.size(), elapsedTime, DEQUE);
}
